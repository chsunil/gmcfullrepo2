...
if ( !class_exists( 'Output_Customization' ) ) {
    class Output_Customization {
        ...
        public function __construct() {
            ...
            add_action( 'jlt_loginfy_login_header', [$this, 'add_extra_div'] );
            add_action( 'login_footer', [$this, 'close_extra_div'] );
            add_action( 'jlt_loginfy_login_footer', [$this, 'close_extra_div'] );
            add_action( 'login_header', [$this, 'add_header_html'] );
            add_action( 'jlt_loginfy_login_header', [$this, 'add_header_html'] );
            add_action( 'login_footer', [$this, 'add_footer_html'] );
            add_action( 'jlt_loginfy_login_footer', [$this, 'add_footer_html'] );
            add_action( 'login_footer', [$this, 'add_video_html'] );
            add_action( 'jlt_loginfy_login_footer', [$this, 'add_video_html'] );
            add_action( 'login_footer', [$this, 'add_slideshow_html'] );
            add_action( 'jlt_loginfy_login_footer', [$this, 'add_slideshow_html'] );
            add_action(
                'loginfy_add_templates',
                [$this, 'add_templates'],
                10,
                1
            );
            ...
            add_action( 'login_enqueue_scripts', [$this, 'jlt_loginfy_login_enqueue_scripts'], 99 );
            add_filter( 'login_body_class', [$this, 'jlt_loginfy_login_body_class'] );
            ...
            add_action( 'login_footer', [$this, 'enable_wp_shake_js'] );
        }

        // Body class
        public function jlt_loginfy_login_body_class( $classes ) {
            $position = !empty( $this->options['credits_logo_position']['background-position'] );
            $position = Helper::jlt_loginfy_class_cleanup( $position );
            $position = 'loginfy-badge-' . esc_attr( $position );
            $classes[] = $position;
            $classes[] = 'loginfy';
            $classes[] = 'loginfy-login-customizer';
            // Logo Settings
            if ( isset( $this->options['logo_settings'] ) && 'text-only' == $this->options['logo_settings'] ) {
                $classes[] = 'loginfy-text-logo';
            }
            if ( isset( $this->options['logo_settings'] ) && 'both' == $this->options['logo_settings'] ) {
                $classes[] = 'loginfy-image-logo loginfy-text-logo';
            }
            if ( isset( $this->options['logo_settings'] ) && 'image-only' == $this->options['logo_settings'] ) {
                $classes[] = 'loginfy-image-logo';
            }
            // Column Widths
            if ( !empty( $this->options['alignment_login_width'] ) && $this->options['alignment_login_width'] == 'fullwidth' ) {
                if ( !empty( $this->options['alignment_login_bg_skew'] ) != 0 ) {
                    $classes[] = 'loginfy-fullwidth';
                }
            }
            if ( !empty( $this->options['alignment_login_width'] ) && $this->options['alignment_login_width'] == 'width_two_column' ) {
                $classes[] = 'loginfy-half-screen';
                if ( isset( $this->options['alignment_login_column'] ) ) {
                    $classes[] = 'jltloginfy-login-' . esc_attr( $this->options['alignment_login_column'] );
                }
            }
            if ( !empty( $this->options['alignment_login_vertical'] ) ) {
                $classes[] = 'loginfy-vertical-align-' . esc_attr( $this->options['alignment_login_vertical'] );
            }
            if ( !empty( $this->options['alignment_login_horizontal'] ) ) {
                $classes[] = 'loginfy-horizontal-align-' . esc_attr( $this->options['alignment_login_horizontal'] );
            }
            if ( !empty( $this->options['jlt_loginfy_login_bg_type'] ) == 'slideshow' ) {
                $classes[] = 'loginfy-slideshow';
            }
            return $classes;
        }
        ...
        public function jlt_loginfy_login_enqueue_scripts() {
            if ( !empty( $this->options['jlt_loginfy_login_bg_type'] ) == 'slideshow' ) {
                wp_enqueue_style( 'loginfy-vegas', LOGINFY_ASSETS . 'vendors/vegas/vegas.min.css' );
                wp_enqueue_script(
                    'loginfy-vegas',
                    LOGINFY_ASSETS . 'vendors/vegas/vegas.min.js',
                    ['jquery'],
                    LOGINFY_VER,
                    true
                );
        ...
        public function login_inline_style() {
            ...
            if ( !is_customize_preview() ) {
                if ( get_option( 'users_can_register' ) == 1 ) {
                    ...
                    if ( !empty( $this->options['login_form_disable_register'] ) ) {
                        $css .= 'body.loginfy-login-customizer p#nav a:nth-child(1) { display:none;}';
                        $css .= 'body.loginfy-login-customizer p#nav { color:transparent !important;}';
                    } else {
                        $css .= 'body.loginfy-login-customizer p#nav a:nth-child(1) { display:revert;}';
                    }
                    // Lost Password
                    if ( !empty( $this->options['login_form_disable_lost_pass'] ) ) {
                        $css .= 'body.loginfy-login-customizer p#nav a:nth-child(2) { display:none;}';
                        $css .= 'body.loginfy-login-customizer p#nav { color:transparent !important;}';
                    } else {
                        $css .= 'body.loginfy-login-customizer p#nav a:nth-child(2) { display:revert;}';
                    }
                } else {
                    // Remember Me
                    if ( !empty( $this->options['login_form_button_remember_me'] ) ) {
                        $css .= 'body.loginfy-login-customizer .forgetmenot { display:none !important;}';
                    }
                    // Lost Password
                    if ( !empty( $this->options['login_form_disable_lost_pass'] ) ) {
                        $css .= 'body.loginfy-login-customizer p#nav { display:none;}';
                    }
                }
            }
            $css .= '
            ...
            .loginfy-badge.left-center {
                right: inherit;
                top: 48.25%;
                bottom: inherit;
                transform: translateY(-50%);
            }
            .loginfy-badge.left-top {
                right: inherit;
                left: 15px;
                top: 15px;
            }
            .loginfy-badge.left-bottom {
                right: inherit;
                left: 20px;
            }
            .loginfy-badge.center-top{
                right: inherit;
                left: 50%;
                top: 0;
                transform: translateX(-50%);
            }
            .loginfy-badge.center-center {
                top: 50%;
                left: 50%;
                bottom: inherit;
                right: inherit;
            ...
            .loginfy-badge.center-bottom{
                right: inherit;
                left: 50%;
                bottom: 15px;
                transform: translateX(-50%);
            }
            .loginfy-badge.right-top {
                top: 15px;
            }
            .loginfy-badge.right-center {
                top: 50%;
                bottom: inherit;
                transform: translateY(-50%);
            }
			...
			.loginfy-badge.middle {
				left: 15px;
				right: 15px;
				margin: 0 auto;
			}

			.loginfy-badge.top-right {
				top: 15px;
				right: 15px;
				bottom: inherit;
			}
            ...
            if ( isset( $this->options['credits_text_color'] ) ) {
                $css .= '.loginfy-badge__text { color:' . esc_attr( $this->options['credits_text_color'] ) . ';}';
            }
            // Icon color.
            if ( isset( $this->options['branding_icon_color'] ) ) {
                $css .= '.loginfy-badge .icon { color:' . esc_attr( $this->options['branding_icon_color'] ) . ';}';
            }
            if ( is_customize_preview() ) {
                $css .= '
				body:not(.loginfy) .loginfy-badge {
        ...
        public function enable_wp_shake_js() {
            if ( !empty( $this->options['login_form_disable_login_shake'] ) ) {
        ...
        public function print_login_styles() {
            ...
            $position_class = Helper::jlt_loginfy_class_cleanup( $position_class );
            // Login Style
        ...
        public function login_error_messages( $error ) {
            ...
            if ( isset( $errors ) && apply_filters( 'wp_adminify_display_custom_errors', true ) ) {
                ...
                if ( $this->options['login_error_messages'] ) {
                    // Username
                    $invalid_username = ( array_key_exists( 'error_incorrect_username', $this->options['login_error_messages'] ) && !empty( $this->options['login_error_messages']['error_incorrect_username'] ) ? $this->options['login_error_messages']['error_incorrect_username'] : sprintf( __( '%1$sError:%2$s Invalid Username.', 'loginfy' ), '<strong>', '</strong>' ) );
                    $empty_username = ( array_key_exists( 'error_empty_username', $this->options['login_error_messages'] ) && !empty( $this->options['login_error_messages']['error_empty_username'] ) ? $this->options['login_error_messages']['error_empty_username'] : sprintf( __( '%1$sError:%2$s The username field is empty.', 'loginfy' ), '<strong>', '</strong>' ) );
                    $username_exists = ( array_key_exists( 'error_exists_username', $this->options['login_error_messages'] ) && !empty( $this->options['login_error_messages']['error_exists_username'] ) ? $this->options['login_error_messages']['error_exists_username'] : sprintf( __( '%1$sError:%2$s This username is already registered. Please choose another one.', 'loginfy' ), '<strong>', '</strong>' ) );
                    // Password
                    $invalid_pasword = ( array_key_exists( 'error_incorrect_password', $this->options['login_error_messages'] ) && !empty( $this->options['login_error_messages']['error_incorrect_password'] ) ? $this->options['login_error_messages']['error_incorrect_password'] : sprintf( __( '%1$sError:%2$s Invalid Password.', 'loginfy' ), '<strong>', '</strong>' ) );
                    $empty_password = ( array_key_exists( 'error_empty_password', $this->options['login_error_messages'] ) && !empty( $this->options['login_error_messages']['error_empty_password'] ) ? $this->options['login_error_messages']['error_empty_password'] : sprintf( __( '%1$sError:%2$s The password field is empty.', 'loginfy' ), '<strong>', '</strong>' ) );
                    // Email
                    $invalid_email = ( array_key_exists( 'error_incorrect_email', $this->options['login_error_messages'] ) && !empty( $this->options['login_error_messages']['error_incorrect_email'] ) ? $this->options['login_error_messages']['error_incorrect_email'] : sprintf( __( '%1$sError:%2$s The email address isn\'t correct..', 'loginfy' ), '<strong>', '</strong>' ) );
                    $empty_email = ( array_key_exists( 'error_empty_email', $this->options['login_error_messages'] ) && !empty( $this->options['login_error_messages']['error_empty_email'] ) ? $this->options['login_error_messages']['error_empty_email'] : sprintf( __( '%1$sError:%2$s Please type your email address.', 'loginfy' ), '<strong>', '</strong>' ) );
                    $email_exists = ( array_key_exists( 'error_exists_email', $this->options['login_error_messages'] ) && !empty( $this->options['login_error_messages']['error_exists_email'] ) ? $this->options['login_error_messages']['error_exists_email'] : sprintf( __( '%1$sError:%2$s This email is already registered, please choose another one.', 'loginfy' ), '<strong>', '</strong>' ) );
                    $invalidcombo = ( array_key_exists( 'invalidcombo_message', $this->options['login_error_messages'] ) && !empty( $this->options['login_error_messages']['invalidcombo_message'] ) ? $this->options['login_error_messages']['invalidcombo_message'] : sprintf( __( '%1$sError:%2$s Invalid username or email.', 'loginfy' ), '<strong>', '</strong>' ) );
                    // Username
        ...
        public function add_header_html() {
            ?>
            <div class="loginfy-background">
                <div class="loginfy-background-wrapper">
        ...
        public function add_video_html() {
            if ( $this->options['jlt_loginfy_login_bg_type'] === "video" ) {
                $video_html = '';
                require dirname( __FILE__ ) . '/footer-video.php';
                return $video_html;
            } else {
                if ( is_customize_preview() ) {
                    printf( '<script type="text/javascript" src="%s?ver=%s"></script>', esc_url( LOGINFY_ASSETS ) . 'vendors/vidim/vidim.min.js', esc_attr( LOGINFY_VER ) );
                }
            }
        }
        ...
        public function add_slideshow_html() {
            if ( !empty( $this->options['jlt_loginfy_login_bg_type'] ) && $this->options['jlt_loginfy_login_bg_type'] === "slideshow" ) {
                $slideshow_html = '';
        ...
        public function add_extra_div() {
            echo '<div class="loginfy-container"><div class="loginfy-form-container">';
        }
