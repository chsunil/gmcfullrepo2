...
class Loginfy_Custom_CSS {
    ...
    public function getStyle() {
        return printf( '<style id="loginfy-customizer-custom-css">%s</style>', esc_attr( $this->getCSS() ) );
    }

}

$customizer_css = new Loginfy_Custom_CSS();
...
if ( $image && $image['url'] != '' ) {
    $customizer_css->addCSS( 'body.loginfy-login-customizer #login h1 a', 'background-image', sprintf( 'url(%s)', $image['url'] ) );
}
/*
 ==============================================
 * Login title Style
    ...
    if ( $module == 'logo_heigh_width' ) {
        ...
        if ( $logo_settings == 'image-only' ) {
            if ( !empty( $module_val['width'] ) ) {
                $customizer_css->addCSS( 'body.loginfy-login-customizer #login h1 a', 'width', $module_val['width'] . $module_val['unit'] . '!important' );
            }
            if ( !empty( $module_val['height'] ) ) {
                $customizer_css->addCSS( 'body.loginfy-login-customizer #login h1 a', 'height', $module_val['height'] . $module_val['unit'] . '!important' );
            }
        }
        if ( $logo_settings == 'both' ) {
            if ( !empty( $module_val['height'] ) ) {
                $customizer_css->addCSS( 'body.loginfy-login-customizer #login h1 a:before', 'height', $module_val['height'] . $module_val['unit'] . '!important' );
            }
        }
    }
    // Logo Padding
    if ( loginfy()->can_use_premium_code__premium_only() && $module == 'logo_padding' ) {
        $unit = ( empty( $module_val['unit'] ) ? 'px' : $module_val['unit'] );
        if ( !empty( $module_val['top'] ) ) {
            $customizer_css->addCSS( 'body.loginfy-login-customizer #login h1', 'padding-top', $module_val['top'] . $unit );
        }
        if ( !empty( $module_val['right'] ) ) {
            $customizer_css->addCSS( 'body.loginfy-login-customizer #login h1', 'padding-right', $module_val['right'] . $unit );
        }
        if ( !empty( $module_val['bottom'] ) ) {
            $customizer_css->addCSS( 'body.loginfy-login-customizer #login h1', 'padding-bottom', $module_val['bottom'] . $unit );
        }
        if ( !empty( $module_val['left'] ) ) {
            $customizer_css->addCSS( 'body.loginfy-login-customizer #login h1', 'padding-left', $module_val['left'] . $unit );
        }
    }
    // Login Title Typography
    if ( loginfy()->can_use_premium_code__premium_only() && $module == 'login_title_typography' ) {
        $selector = 'body.loginfy-login-customizer #login h1 a';
 ...
 * ============================================== */
$bg_type = $this->options['jlt_loginfy_login_bg_type'];
$bg_overlay_type = ( !empty( $this->options['jlt_loginfy_login_bg_overlay_type'] ) ? $this->options['jlt_loginfy_login_bg_overlay_type'] : '' );
$bg_overlay_color = ( !empty( $this->options['jlt_loginfy_login_bg_overlay_color'] ) ? $this->options['jlt_loginfy_login_bg_overlay_color'] : '' );
$bg_overlay_g_color = ( !empty( $this->options['jlt_loginfy_login_bg_overlay_gradient_color'] ) ? $this->options['jlt_loginfy_login_bg_overlay_gradient_color'] : '' );
$overlay_opacity = ( !empty( $this->options['jlt_loginfy_login_overlay_opacity'] ) ? $this->options['jlt_loginfy_login_overlay_opacity'] : '' );
$selector = 'body.loginfy-login-customizer .login-background';
if ( $bg_type == 'color_image' ) {
    $bg_color_opt = $this->options['jlt_loginfy_login_bg_color_opt'];
    if ( $bg_color_opt == 'color' ) {
        $bg_color = $this->options['jlt_loginfy_login_bg_color'];
    } else {
        $gradient_bg = $this->options['jlt_loginfy_login_gradient_bg'];
    }
}
if ( $bg_type == 'color_image' ) {
    if ( $bg_color_opt == 'color' ) {
        ...
        if ( loginfy()->can_use_premium_code__premium_only() && !empty( $gradient_bg ) ) {
...
if ( loginfy()->can_use_premium_code__premium_only() && $bg_overlay_type == 'gradient' && !empty( $bg_overlay_g_color ) ) {
...
$selector_fullwidth = 'body.loginfy-login-customizer.loginfy-fullwidth .loginfy-form-container:after';
$selector_half = 'body.loginfy-login-customizer.loginfy-half-screen .loginfy-container:before';
$selector = ( $login_width == 'fullwidth' ? $selector_fullwidth : $selector_half );
if ( loginfy()->can_use_premium_code__premium_only() && $login_width == 'fullwidth' ) {
    if ( $login_bg_skew > 0 ) {
...
} elseif ( loginfy()->can_use_premium_code__premium_only() && $login_bg_type == 'gradient' && !empty( $login_bg_g_color ) ) {
...
$selector_login_form = 'body.loginfy-login-customizer #loginform, body.loginfy-login-customizer #lostpasswordform, body.loginfy-login-customizer #registerform';
$selector = ( in_array( $template, loginfyGetBoxedFormTemplates() ) ? $selector_login : $selector_login_form );
// Background Color
...
// Background Gradient
if ( loginfy()->can_use_premium_code__premium_only() && $login_form_bg_type == 'gradient' && !empty( $login_form_bg_gradient ) ) {
...
// Margin
if ( loginfy()->can_use_premium_code__premium_only() && !empty( $login_form_margin ) ) {
...
// Padding
if ( loginfy()->can_use_premium_code__premium_only() && !empty( $login_form_padding ) ) {
...
// Border Radius
if ( loginfy()->can_use_premium_code__premium_only() && !empty( $login_form_border_radius ) ) {
...
// Box Shadow
if ( loginfy()->can_use_premium_code__premium_only() && !empty( $login_form_box_shadow ) && !empty( $login_form_box_shadow['bs_color'] ) ) {
...
foreach ( $login_form_fields as $setting => $value ) {
    ...
    switch ( $setting ) {
        ...
        case 'style_fields_color':
            ...
            break;
        case 'style_border':
            if ( loginfy()->can_use_premium_code__premium_only() && !empty( $value ) ) {
...
foreach ( $button_settings as $setting => $value ) {
    switch ( $setting ) {
        ...
        case 'button_text_shadow':
            if ( loginfy()->can_use_premium_code__premium_only() && !empty( $value['ts_color'] ) ) {
        ...
        case 'button_box_shadow':
            if ( loginfy()->can_use_premium_code__premium_only() && !empty( $value['bs_color'] ) ) {
