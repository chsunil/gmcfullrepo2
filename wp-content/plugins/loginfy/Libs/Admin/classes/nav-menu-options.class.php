 ...
 *
 */
if ( ! class_exists( 'LOGINFY_Nav_Menu_Options' ) ) {
  class LOGINFY_Nav_Menu_Options extends LOGINFY_Abstract{
    ...
    public function wp_edit_nav_menu_walker( $class, $menu_id ) {
      ...
      if( version_compare( $wp_version, '5.4.0', '<' ) ) {
        ...
        if ( ! class_exists( 'LOGINFY_Walker_Nav_Menu_Edit' ) ) {
          LOGINFY::include_plugin_file( 'functions/walker.php' );
        }

        return 'LOGINFY_Walker_Nav_Menu_Edit';
    ...
    public function wp_nav_menu_item_custom_fields( $menu_item_id, $item, $depth, $args ) {
      ...
      $errors = ( ! empty( $menu_item_id ) ) ? get_post_meta( $menu_item_id, '_loginfy_errors_'. $this->unique, true ) : array();
      $errors = ( ! empty( $errors ) ) ? $errors : array();
      $class  = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_post_meta( $menu_item_id, '_loginfy_errors_'. $this->unique );
      }

      echo '<div class="loginfy loginfy-nav-menu-options'. esc_attr( $class ) .'">';

        foreach ( $this->sections as $section ) {

          $section_icon  = ( ! empty( $section['icon'] ) ) ? '<i class="loginfy-nav-menu-icon '. esc_attr( $section['icon'] ) .'"></i>' : '';
          $section_title = ( ! empty( $section['title'] ) ) ? $section['title'] : '';

          echo '<div class="loginfy-fields">';

          echo ( $section_title || $section_icon ) ? '<div class="loginfy-nav-menu-title"><h4>'. $section_icon . $section_title .'</h4></div>' : '';
          ...
          if ( ! empty( $section['fields'] ) ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              LOGINFY::field( $field, $this->get_meta_value( $menu_item_id, $field ), $this->unique .'['. $menu_item_id .']', 'menu' );
    ...
    public function wp_update_nav_menu_item( $menu_id, $menu_item_db_id, $menu_item_args ) {
      ...
      $data = apply_filters( "loginfy_{$this->unique}_save", $data, $menu_item_db_id, $this );

      do_action( "loginfy_{$this->unique}_save_before", $data, $menu_item_db_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_post_meta( $menu_item_db_id, '_loginfy_errors_'. $this->unique, $errors );
        }

      }

      do_action( "loginfy_{$this->unique}_saved", $data, $menu_item_db_id, $this );

      do_action( "loginfy_{$this->unique}_save_after", $data, $menu_item_db_id, $this );
