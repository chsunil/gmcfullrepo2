...
if ( ! class_exists( 'Recommended' ) ) {
	...
	class Recommended {
		...
		public function __construct( $menu_order = 99 ) {
			...
			add_action( 'wp_ajax_jlt_loginfy_recommended_upgrade_plugin', array( $this, 'jlt_loginfy_recommended_upgrade_plugin' ) );
			add_action( 'wp_ajax_jlt_loginfy_recommended_activate_plugin', array( $this, 'jlt_loginfy_recommended_activate_plugin' ) );
		}
		...
		public function render_recommended_plugins() { ?>
			<div class='loginfy-recommended-wrapper'>
				<?php $this->header(); ?>
				<?php $this->body(); ?>
			</div>
			<?php
		...
		public function header() {
			?>
			<div class='loginfy-recommended-header'>
				<div class='loginfy-recommended-title'>
					<h2>
						<?php echo esc_html__( 'Recommended Plugins', 'loginfy' ); ?>
					</h2>
				</div>
				<div class='loginfy-recommended-menu'>
					<div class="wp-filter">
						...
						<form class="search-form loginfy-search-plugins mr-0" method="get">
							<input type="hidden" name="tab" value="search">
							<label class="screen-reader-text" for="search-plugins">
								<?php echo esc_html__( 'Search Plugins', 'loginfy' ); ?>
							</label>
							<input type="search" name="s" id="search-plugins" value="" class="wp-filter-search" placeholder="<?php echo esc_html__( 'Search plugins...', 'loginfy' ); ?>">
							<input type="submit" id="search-submit" class="button hide-if-js" value="<?php echo esc_html__( 'Search Plugins', 'loginfy' ); ?>">
						</form>
					</div>
				</div>
			</div>
								...
								if ( 'install' === $install_status['status'] ) {
									...
									<li class="mr-0">
										<button class="install-now button button-primary" data-install-url="<?php echo esc_attr( $plugin['download_link'] ); ?>">
											<?php echo esc_html__( 'Install Now', 'loginfy' ); ?>
										</button>
									</li>
									<?php
								} elseif ( 'update_available' === $install_status['status'] ) {
									...
									<li class="mr-0">
										<button class="update-now button" data-plugin="<?php echo esc_attr( $install_status['file'] ); ?>" data-slug="<?php echo esc_attr( $plugin['slug'] ); ?>" data-update-url="<?php echo esc_attr( $install_status['url'] ); ?>">
											<?php echo esc_html__( 'Update Now', 'loginfy' ); ?>
										</button>
									</li>
									<?php
								} elseif ( ( 'latest_installed' === $install_status['status'] ) || ( 'newer_installed' === $install_status['status'] ) ) {
									if ( is_plugin_active( $install_status['file'] ) ) {
										...
										<li class="mr-0">
											<button type="button" class="button button-disabled" disabled="disabled">
												<?php echo esc_html__( 'Activated', 'loginfy' ); ?>
											</button>
										</li>
										<?php
									} elseif ( current_user_can( 'activate_plugin', $install_status['file'] ) ) {
										...
										<button class="button activate-now" data-plugin-file="<?php echo esc_attr( $install_status['file'] ); ?>">
											<?php echo esc_html__( 'Activate', 'loginfy' ); ?>
										</button>
										<?php
									} else {
										?>
										<li class="mr-0">
											<button type="button" class="button button-disabled" disabled="disabled">
												<?php echo esc_html__( 'Installed', 'loginfy' ); ?>
											</button>
										</li>
										<?php
									}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jlt_loginfy_recommended_activate_plugin() {
			try {
				if ( isset( $_POST['file'] ) ) {
					$nonce = isset( $_POST['nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['nonce'] ) ) : '';

					if ( ! wp_verify_nonce( $nonce, 'jlt_loginfy_recommended_nonce' ) ) {
						wp_send_json_error( array( 'mess' => esc_html__( 'Nonce is invalid', 'loginfy' ) ) );
					}

					if ( ( is_multisite() && ! is_network_admin() ) || ! current_user_can( 'install_plugins' ) ) {
						wp_send_json_error( array( 'mess' => esc_html__( 'Invalid access', 'loginfy' ) ) );
					}
					...
					if ( ! in_array( $plugin, $plugin_links ) ) {
						wp_send_json_error( array( 'mess' => esc_html__( 'Invalid plugin', 'loginfy' ) ) );
					}
					...
					wp_send_json_success(
						array(
							'mess' => esc_html__( 'Activate success', 'loginfy' ),
						)
					);
				}
			} catch ( \Error $ex ) {
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jlt_loginfy_recommended_upgrade_plugin() {
			try {
				require_once ABSPATH . 'wp-admin/includes/plugin-install.php';
					...
					if ( 'install' === $type ) {
						...
						if ( $plugin_data && ! is_wp_error( $plugin_data ) ) {
							...
							if ( is_wp_error( $active_plugin ) ) {
								...
								wp_send_json_success(
									array(
										'mess' => esc_html__( 'Install success', 'loginfy' ),
									)
								);
							}
						} else {
						...
						if ( is_wp_error( $result ) ) {
							...
							wp_send_json_success(
								array(
									'mess'   => esc_html__( 'Update success', 'loginfy' ),
									'active' => $is_active,
								)
							);
						}
				...
				wp_send_json_error(
					array(
						'mess' => esc_html__( 'Error exception.', 'loginfy' ),
						array(
							'error' => $ex,
						),
					)
