...
if ( ! function_exists( 'trx_addons_elm_add_params_fly' ) ) {
	...
	function trx_addons_elm_add_params_fly( $element, $section_id, $args ) {
		...
		if ( in_array( $element->get_name(), array( 'section', 'column', 'common', 'common-optimized' ) ) && $section_id == '_section_responsive' ) {
			...
			$element->start_controls_section( 'section_trx_fly', array(
																		...
																		'label' => __( 'Fly', 'trx_addons' )	// . trx_addons_get_theme_doc_link( '#elementor_extension_fly' ),
																	) );
