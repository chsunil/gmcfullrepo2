<?php
/**
 * QMS – F-16s2 Audit Programme (Surveillance Year 2)
 * ACF Group: group_e18f7c62d39c
 ...
 *   s2_technical_area — clone of field_qms_f02_technical_area
 *   s2_AUDIT_PROGRAMMEF16 — matrix_flexible
 */
...
if ( ! function_exists('f16s2_val') ) {
    function f16s2_val( $v, $fb = '-' ) {
...
$org       = ( $org_raw && ! is_array($org_raw) ) ? esc_html($org_raw)
           : ( is_array($org_raw) ? f16s2_val($org_raw) : esc_html( get_post_field('post_title', $post_id) ) );
$ref_no    = f16s2_val( get_field( 's2_ref_no', $post_id ) );
$standard  = f16s2_val( get_field( 'standard_applied', $post_id ) );
$tech_area = f16s2_val( get_field( 's2_technical_area', $post_id ) );
$matrix    = get_field( 's2_AUDIT_PROGRAMMEF16', $post_id );
?><!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<h2>F-16s2 &nbsp;|&nbsp; QMS Surveillance Year 2 &nbsp;|&nbsp; Version 1.00</h2>
