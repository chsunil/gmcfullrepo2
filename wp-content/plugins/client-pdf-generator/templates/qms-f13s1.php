<?php
/**
 * QMS – F-13s1 Attendance Sheet (Surveillance Year 1)
 * ACF Group: group_6974d1c3c4539
...
if ( ! function_exists('f13s1_val') ) {
    function f13s1_val( $v, $fb = '-' ) {
...
$org     = ( $org_raw && ! is_array($org_raw) ) ? esc_html($org_raw)
         : ( is_array($org_raw) ? f13s1_val($org_raw) : esc_html( get_post_field('post_title', $post_id) ) );

$ref_no   = f13s1_val( get_field( 'f13Ref_No', $post_id ) );
$date_raw = get_field( 'f13Date', $post_id );
...
<h2>F-13s1 &nbsp;|&nbsp; QMS Surveillance Year 1 &nbsp;|&nbsp; Version 1.00</h2>
