 ...
 *   location        — clone of field_address_on_site
 *   audit_type      — radio
 *   feedback_f24    — matrix_flexible
 *   suggestions_or_any_other_points:
 *   do_you_have_any_plan_for_certification_in_any_other_systems_if_yes_please_inform:
 *   name            — clone of field_67d5ac7dd1e5c
 *   evaluation_result — select
 *   reviewed_by     — group (reviewed_by, signature, comment)
 */
...
if ( ! function_exists('f24_val') ) {
    function f24_val( $v, $fb = '-' ) {
...
$org     = ( $org_raw && ! is_array($org_raw) ) ? esc_html($org_raw)
         : ( is_array($org_raw) ? f24_val($org_raw) : esc_html( get_post_field('post_title', $post_id) ) );

$location    = f24_val( get_field( 'location', $post_id ) );
$audit_type  = f24_val( get_field( 'audit_type', $post_id ) );
$suggestions = f24_val( get_field( 'suggestions_or_any_other_points:', $post_id ) );
$cert_plans  = f24_val( get_field( 'do_you_have_any_plan_for_certification_in_any_other_systems_if_yes_please_inform:', $post_id ) );
$client_name = f24_val( get_field( 'name', $post_id ) );
$eval_result = f24_val( get_field( 'evaluation_result', $post_id ) );

$rev = get_field( 'reviewed_by', $post_id );
$rev_name = is_array($rev) ? f24_val( $rev['reviewed_by'] ?? '' ) : '-';
$rev_sig  = is_array($rev) ? f24_val( $rev['signature'] ?? '' ) : '-';
$rev_cmt  = is_array($rev) ? f24_val( $rev['comment'] ?? '' ) : '-';
...
<h2>F-24 &nbsp;|&nbsp; QMS Certification &nbsp;|&nbsp; Version 1.00</h2>
...
<?php if ( is_array($matrix) && ! empty($matrix) ) :
    // Render matrix rows as a simple table
    $first = reset($matrix);
    $cols  = is_array($first) ? array_keys($first) : [];
?>
<table>
    <?php if ( $cols ) : ?>
    <thead><tr><?php foreach ($cols as $c) : ?><th><?= esc_html($c) ?></th><?php endforeach; ?></tr></thead>
    <?php endif; ?>
    <tbody>
