<?php
/**
 * QMS F-05S1 — Surveillance Audit-1 Team Allocation Plan
 * ACF Group: group_6970b191c8d75
 *
 * Seamless clones (read via source meta key):
 *   organization_name        ← field_org_name
 *   proposal_ref_no          ← field_68554bdf55898 (clone name: f05s1gmcspl_ref_no, display: group)
...
// ── helpers ───────────────────────────────────────────────────────────────────
function f05s1_fmt( $raw, $format = 'd/m/Y' ) {
    if ( empty( $raw ) ) return '';
    $ts = strtotime( $raw );
    return $ts ? date( $format, $ts ) : esc_html( $raw );
}

function f05s1_cb( $checked = false ) {
    return $checked ? '[X]' : '[  ]';
}
...
$exclusions     = esc_html( get_field( 'exclusions', $post_id ) ?: '' );

// Prime contact person — unnamed group key field_69765217382e2
$contact     = get_field( 'field_69765217382e2', $post_id ) ?: [];
...
// ── pull own fields ───────────────────────────────────────────────────────────
$audit_objective = get_field( 'audit_objective', $post_id ) ?: '';

// Audit dates group
$audit_dates_grp  = get_field( 'audit_dates', $post_id ) ?: [];
$audit_date_onsite = gmc_format_date(
    $audit_dates_grp['stage1_intimation_date_yearly_surveillance_intimation_for_s1_&_s2_surv1']
...
// Audit team repeater
$audit_team = get_field( 'audit_team_allocation_plan', $post_id ) ?: [];

// Prepared/Approved — unnamed group key field_6976e70b30626
$prep_appr = get_field( 'field_6976e70b30626', $post_id ) ?: [];
...
<head>
<meta charset="UTF-8">
<title>F-05S1 Surveillance Audit-1 Team Allocation Plan</title>
<style>
...
<tr>
    ...
    <td class="title-cell">
        ...
        <div style="font-size:8pt;">Surveillance Audit &#8211; I</div>
    </td>
    <td class="cb-cell">
        <div class="cb-list">
            <div><?= f05s1_cb(false) ?> Stage-1</div>
            <div><?= f05s1_cb(false) ?> Stage-2</div>
            <div><?= f05s1_cb(true)  ?> Surveillance Audit (Surv1)</div>
            <div><?= f05s1_cb(false) ?> Surveillance Audit (Surv2)</div>
            <div><?= f05s1_cb(false) ?> Re Certification</div>
        </div>
    </td>
</tr>
</table>
...
<tr class="footer-row">
    <td colspan="8">
        ...
        <strong>F-05S1 (Version 1.00)</strong>
    </td>
</tr>
</table>
