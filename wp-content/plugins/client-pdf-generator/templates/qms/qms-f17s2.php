<?php
/**
 * QMS – F-17s2 On-Going Surveillance Plan (Surveillance Year 2)
 * ACF Group: group_7ce695799b3a
 *
 * Seamless clone fields (prefix_name=0) — source meta keys:
 *   f17s2organization → organization_name
 *   f17s2ref_no       → proposal_ref_no
 *   f17s2standard     → cert_scheme
 *   f17s2location     → head_office
 *
 * Group: f17s2planned_monthyear  — stage_i, stage_2, 1st_Surveillance, 2nd_Surveillance, Re-Certification
 * Group: f17s2executed_monthyear — same sub-field structure
 * Matrix: f17s2area_process (matrix_flexible) — status (radio), Off1st (text)
 * Textarea: f17s2remarks
 */
...
if ( ! function_exists('f17s2_val') ) {
    function f17s2_val( $v, $fb = '-' ) {
...
// ── Planned (Month/Year) ───────────────────────────────────────────────────────
$plan_grp    = get_field( 'f17s2planned_monthyear', $post_id ) ?: [];
$plan_s1     = f17s2_val( $plan_grp['stage_i']         ?? '' );
$plan_s2     = f17s2_val( $plan_grp['stage_2']         ?? '' );
$plan_surv1  = f17s2_val( $plan_grp['1st_Surveillance'] ?? '' );
$plan_surv2  = f17s2_val( $plan_grp['2nd_Surveillance'] ?? '' );
$plan_recert = f17s2_val( $plan_grp['Re-Certification'] ?? '' );

// ── Executed (Month/Year) ──────────────────────────────────────────────────────
$exec_grp    = get_field( 'f17s2executed_monthyear', $post_id ) ?: [];
$exec_s1     = f17s2_val( $exec_grp['stage_i']         ?? '' );
$exec_s2     = f17s2_val( $exec_grp['stage_2']         ?? '' );
$exec_surv1  = f17s2_val( $exec_grp['1st_Surveillance'] ?? '' );
$exec_surv2  = f17s2_val( $exec_grp['2nd_Surveillance'] ?? '' );
$exec_recert = f17s2_val( $exec_grp['Re-Certification'] ?? '' );

// ── Area/Process matrix ────────────────────────────────────────────────────────
$matrix  = get_field( 'f17s2area_process', $post_id );
$remarks = esc_html( get_field( 'f17s2remarks', $post_id ) ?: '' );
?><!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<table style="margin-bottom:4px;">
    <tr>
        ...
        <td class="no-border" style="width:22%; font-size:8px; vertical-align:top; padding-top:2px;">
            <strong>F-17s2 (Version 2.00, 20.03.2016)</strong><br>QMS Surveillance Year 2
        </td>
    </tr>
</table>
