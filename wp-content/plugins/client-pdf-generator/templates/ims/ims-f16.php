<?php
/**
 * IMS – F-16 Audit Programme
 * ACF Group: group_ims_f16
 */

if ( ! defined('ABSPATH') ) exit;

$post_id = $args['post_id'] ?? get_the_ID();

// ── Prefix Logic ─────────────────────────────────────────────────────────────
$prefix = $args['prefix'] ?? ''; // e.g. 'f16s1'

// ── Helpers ───────────────────────────────────────────────────────────────────
$org_raw = get_field( $prefix . 'organization_name', $post_id );
$org     = ( $org_raw && ! is_array($org_raw) ) ? esc_html($org_raw)
         : esc_html( get_post_field('post_title', $post_id) );

$ref_no    = get_field($prefix . 'refno', $post_id) ?: get_field($prefix . 'proposal_ref_no', $post_id) ?: '-';
$standard  = get_field($prefix . 'standards', $post_id) ?: get_field('cert_scheme', $post_id) ?: '-';
$tech_area = get_field($prefix . 'technical_code_area', $post_id) ?: get_field('technical_code_area', $post_id) ?: '-';

$matrix = get_field( 'AUDIT_PROGRAMMEF16', $post_id ) ?: [];

$logo_b64 = 'data:image/jpeg;base64,...'; // Omitted
?>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<style>
    @page { size: A4 portrait; margin: 12mm 10mm; }
    body { font-family: Arial, sans-serif; font-size: 8.5px; color: #333; line-height: 1.2; }
    table { width: 100%; border-collapse: collapse; margin-bottom: 6px; }
    th, td { border: 1px solid #444; padding: 4px; vertical-align: middle; text-align: left; }
    th { background: #eaeff2; font-weight: bold; text-align: center; font-size: 8px; }
    
    .h-logo { border: none; width: 15%; text-align: center; }
    .h-title { border: none; text-align: center; font-size: 14pt; font-weight: bold; color: #2c3e50; }
    .h-form { border: none; text-align: right; font-size: 8px; vertical-align: bottom; width: 22%; color: #7f8c8d; }

    .lbl { background: #f8f9fa; font-weight: bold; width: 22%; }
    .center { text-align: center; }
    .note-box { font-size: 7.5px; border: 1px solid #ccc; padding: 5px; background: #fafafa; margin-top: 5px; }
</style>
</head>
<body>

<table>
    <tr>
        <td class="h-logo"><img src="data:image/jpeg;base64, " alt="Logo"></td>
        <td class="h-title">AUDIT PROGRAMME<br><span style="font-size:10px; font-weight:normal; color:#777;">IMS (9001, 14001 & 45001)</span></td>
        <td class="h-form">F-16<br><strong>(Version 5.00)</strong></td>
    </tr>
</table>

<table>
    <tr><td class="lbl">Organization</td><td colspan="3" style="font-weight:bold;"><?= $org ?></td><td class="lbl">Ref. No.</td><td><?= $ref_no ?></td></tr>
    <tr><td class="lbl">Standard</td><td><?= $standard ?></td><td class="lbl">Technical Area</td><td colspan="3"><?= $tech_area ?></td></tr>
</table>

<table style="margin-top:10px;">
    <thead>
        <tr>
            <th style="width:25%;">Area / Process</th>
            <th>Stage I</th>
            <th>Stage II</th>
            <th>1st Surv</th>
            <th>2nd Surv</th>
            <th>Remarks</th>
        </tr>
    </thead>
    <tbody>
        <?php if (!empty($matrix)) : ?>
            <?php foreach ($matrix as $row => $cols) : ?>
            <tr>
                <td class="lbl"><?= esc_html($row) ?></td>
                <td class="center"><?= esc_html($cols['stage1'] ?? '') ?></td>
                <td class="center"><?= esc_html($cols['stage2'] ?? '') ?></td>
                <td class="center"><?= esc_html($cols['surv1'] ?? '') ?></td>
                <td class="center"><?= esc_html($cols['surv2'] ?? '') ?></td>
                <td><?= esc_html($cols['remarks'] ?? '') ?></td>
            </tr>
            <?php endforeach; ?>
        <?php else : ?>
            <tr><td colspan="6" class="center">No data available.</td></tr>
        <?php endif; ?>
    </tbody>
</table>

<div class="note-box">
    <strong>Legend:</strong> [X] Planned / Audited | [+] Major NC | [-] Minor NC | [O] Observation<br>
    This programme covers the initial 3-year certification cycle.
</div>

</body>
</html>
