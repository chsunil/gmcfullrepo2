<?php
/**
 * IMS – F-06 Documentation Review Report
 * ACF Group: group_ims_f06
 */

if ( ! defined('ABSPATH') ) exit;

$post_id = $args['post_id'] ?? get_the_ID();

// ── Safe field helper ─────────────────────────────────────────────────────────
if ( ! function_exists('ims_f06v') ) {
    function ims_f06v( $key, $post_id, $fallback = '-' ) {
        $v = get_field( $key, $post_id );
        if ( empty($v) ) return $fallback;
        if ( is_array($v) ) {
            if ( isset($v['display_name']) ) return esc_html( $v['display_name'] );
            $flat = array_filter( array_map( fn($i) => is_string($i) ? $i : ( $i['label'] ?? $i['value'] ?? '' ), $v ) );
            return esc_html( implode(', ', $flat) ) ?: $fallback;
        }
        return esc_html( $v );
    }
}

// ── Header fields ─────────────────────────────────────────────────────────────
$org = function_exists('gmc_get_organization_name')
     ? gmc_get_organization_name( $post_id )
     : ims_f06v( 'organization_name', $post_id );

if ( empty( $org ) || $org === '-' ) {
    $org = get_post_meta( $post_id, 'organization_name', true ) ?: get_the_title( $post_id );
}

$raw_date = get_field( 'stage1_audit_initial', $post_id );
$date     = $raw_date ? date( 'd/m/Y', strtotime( $raw_date ) ) : '-';

$standard = ims_f06v( 'cert_scheme', $post_id );

$lead_auditor = get_field( 'lead_auditor', $post_id ) ?: [];
if ( is_array( $lead_auditor ) && ! empty( $lead_auditor ) ) {
    $auditors = $lead_auditor['signature']
             ?? $lead_auditor['lead_auditor_name']
             ?? $lead_auditor['auditor_name']
             ?? $lead_auditor['name']
             ?? implode( ', ', array_filter( array_map( fn($v) => is_string($v) ? trim($v) : '', $lead_auditor ) ) )
             ?: '-';
} else {
    $auditors = ( is_string($lead_auditor) && $lead_auditor ) ? $lead_auditor : '-';
}

$sites    = ims_f06v( 'main_operative_site', $post_id );
$qm_date  = ims_f06v( 'date_of_current_issue', $post_id );
$issue_no = get_field( 'issue_no:', $post_id ) ?: '-';

// ── Matrix field data (IMS Integrated Clauses) ───────────────────────────────
$matrix_raw = get_field( 'field_ims_f06_integrated_clauses', $post_id ) ?: [];
$matrix = [];
foreach ( $matrix_raw as $row_key => $cols ) {
    $parts  = explode( "\t", $row_key, 2 );
    $clause = trim( $parts[0] );
    if ( $clause !== '' ) {
        $matrix[ $clause ] = $cols;
    }
}

function ims_f06_cell( $matrix, $key, $col ) {
    return isset( $matrix[ $key ][ $col ] ) ? esc_html( $matrix[ $key ][ $col ] ) : '';
}

// ── IMS Integrated Clause definitions ────────────────────────────────────────
$clauses = [
    [ '4',      'Context of the Organization',                                               true  ],
    [ '4.1',    'Understanding the Organization and its Context',                            false ],
    [ '4.2',    'Understanding the needs and expectations of interested parties',            false ],
    [ '4.3',    'Determining the scope of the Integrated management system',                 false ],
    [ '4.4',    'Integrated management system',                                              false ],
    [ '5',      'Leadership',                                                                true  ],
    [ '5.1',    'Leadership and Commitment',                                                 false ],
    [ '5.2',    'IMS Policy',                                                                false ],
    [ '5.3',    'Organizational roles, responsibilities and authorities',                    false ],
    [ '5.4',    'Consultation and participation of workers',                                 false ],
    [ '6',      'Planning',                                                                  true  ],
    [ '6.1',    'Actions to address risks and opportunities',                                false ],
    [ '6.1.2',  'Environmental Aspects & Hazard identification and risk assessment',         false ],
    [ '6.1.3',  'Compliance obligation / Legal and other requirements',                      false ],
    [ '6.1.4',  'Planning action',                                                           false ],
    [ '6.2',    'IMS objectives and planning to achieve them',                               false ],
    [ '6.3',    'Planning of changes',                                                       false ],
    [ '7',      'Support',                                                                   true  ],
    [ '7.1',    'Resources',                                                                 false ],
    [ '7.2',    'Competence',                                                                false ],
    [ '7.3',    'Awareness',                                                                 false ],
    [ '7.4',    'Communication',                                                             false ],
    [ '7.5',    'Documented Information',                                                    false ],
    [ '8',      'Operations',                                                                true  ],
    [ '8.1',    'Operational Planning and Control',                                          false ],
    [ '8.2',    'Emergency Prep & Response | Requirements for Products/Services',            false ],
    [ '8.3',    'Design and Development of Products and Services',                           false ],
    [ '8.4',    'Control of externally provided processes/products',                          false ],
    [ '8.5',    'Production and Service Provision',                                          false ],
    [ '8.6',    'Release of Products and Services',                                          false ],
    [ '8.7',    'Control of nonconforming outputs',                                          false ],
    [ '9',      'Performance evaluation',                                                    true  ],
    [ '9.1',    'Monitoring, measurement, analysis and evaluation',                          false ],
    [ '9.1.2',  'Evaluation of compliance & Customer Satisfaction',                          false ],
    [ '9.1.3',  'Analysis and evaluation',                                                   false ],
    [ '9.2',    'Internal Audit',                                                            false ],
    [ '9.3',    'Management Review',                                                         false ],
    [ '10',     'Improvement',                                                               true  ],
    [ '10.1',   'General',                                                                   false ],
    [ '10.2',   'Non Conformity and Corrective action',                                      false ],
    [ '10.3',   'Continual Improvement',                                                     false ],
];

$logo_b64 = 'data:image/jpeg;base64,';
?>
<!doctype html>
<html>
<head>
  <meta charset="utf-8">
  <style>
    @page { margin: 15mm 12mm 12mm 12mm; }
    body  { font-family: Arial, sans-serif; font-size: 10px; color: #000; margin: 0; padding: 0; line-height: 1.4; }

    table { width: 100%; border-collapse: collapse; margin-bottom: 6px; }
    th, td { border: 1px solid #000; padding: 4px 5px; vertical-align: top; }
    th { font-weight: bold; text-align: left; background: #2c3e50; color: #fff; }

    /* Header */
    .h-logo  { width: 14%; border: none; text-align: center; vertical-align: middle; }
    .h-logo img { max-width: 75px; max-height: 65px; }
    .h-title { text-align: center; font-size: 14px; font-weight: bold; vertical-align: middle; color: #1a252f; }
    .h-form  { text-align: right; font-size: 9px; vertical-align: bottom; white-space: nowrap; color: #7f8c8d; }
    .no-b    { border: none !important; background: transparent !important; }

    /* Info rows */
    .lbl { font-weight: bold; width: 22%; background: #f8f9fa; color: #2c3e50; }
    .val { color: #2c3e50; }

    /* Clause table */
    .cl-no   { width: 8%;  text-align: center; font-weight: bold; }
    .cl-req  { width: 50%; }
    .cl-comp { width: 12%; text-align: center; }
    .cl-comm { width: 30%; }
    .cl-sect { background: #ecf0f1; font-weight: bold; color: #2c3e50; font-size: 10.5px; border-left: 4px solid #34495e; }
    .yes     { font-weight: bold; color: #27ae60; }
    .no      { font-weight: bold; color: #e74c3c; }

    .branding-sub { font-size: 9px; font-weight: normal; color: #7f8c8d; display: block; margin-top: 2px; }
  </style>
</head>
<body>

<!-- ══ HEADER ══════════════════════════════════════════════════════════════ -->
<table>
  <tr>
    <td class="h-logo no-b">
      <img src="<?= $logo_b64 ?>" alt="Logo">
    </td>
    <td class="h-title no-b" style="text-align: center;">
      DOCUMENTATION REVIEW REPORT
      <span class="branding-sub">IMS (QMS, EMS & OH&SMS)</span>
    </td>
    <td class="h-form no-b">F-06<br><strong>(Version 5.00, 30.10.2023)</strong></td>
  </tr>
</table>

<!-- ══ INFO TABLE ══════════════════════════════════════════════════════════ -->
<table>
  <tr>
    <td class="lbl">Organization</td>
    <td colspan="3" class="val"><?= esc_html($org) ?></td>
    <td class="lbl">Date</td>
    <td class="val"><?= esc_html($date) ?></td>
  </tr>
  <tr>
    <td class="lbl">Standard</td>
    <td colspan="3" class="val"><?= $standard ?></td>
    <td class="lbl">Auditor(s)</td>
    <td class="val"><?= esc_html($auditors) ?></td>
  </tr>
  <tr>
    <td class="lbl">Sites</td>
    <td colspan="5" class="val"><?= $sites ?></td>
  </tr>
  <tr>
    <td class="lbl">Integrated Manual Date</td>
    <td colspan="3" class="val"><?= $qm_date ?></td>
    <td class="lbl">Issue No.</td>
    <td class="val"><?= $issue_no ?></td>
  </tr>
</table>

<!-- ══ REQUIREMENT TABLE ═══════════════════════════════════════════════════ -->
<table>
  <thead>
    <tr>
      <th class="cl-no">Clause</th>
      <th class="cl-req">Integrated Requirements (ISO 9001/14001/45001)</th>
      <th class="cl-comp">Compliance</th>
      <th class="cl-comm">Comments / Evidence Reference</th>
    </tr>
  </thead>
  <tbody>
    <?php foreach ( $clauses as [ $key, $label, $is_header ] ) : ?>
      <?php if ( $is_header ) : ?>
        <tr>
          <td colspan="4" class="cl-sect"><?= esc_html($key) ?>.&nbsp; <?= esc_html($label) ?></td>
        </tr>
      <?php else :
        $compliance = ims_f06_cell( $matrix, $key, 'Compliance' );
        $comments   = ims_f06_cell( $matrix, $key, 'ims_f06_comments' );
      ?>
        <tr>
          <td class="cl-no"><?= esc_html($key) ?></td>
          <td class="cl-req"><?= esc_html($label) ?></td>
          <td class="cl-comp <?= $compliance === 'Yes' ? 'yes' : ( $compliance === 'No' ? 'no' : '' ) ?>">
            <?= $compliance ?: '&nbsp;' ?>
          </td>
          <td class="cl-comm"><?= nl2br( $comments ) ?></td>
        </tr>
      <?php endif; ?>
    <?php endforeach; ?>
  </tbody>
</table>

</body>
</html>
