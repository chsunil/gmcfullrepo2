<?php
/**
 * QMS – F-17s2 On Going Surveillance Plan (Year 2)
 * ACF Group: group_7ce695799b3a
 * Fields: s2_organization, s2_ref_no, s2_standard, s2_location,
 *   s2_planned_monthyear (group), s2_Executed_monthyear_copy (group), s2_area_process (matrix)
 */
...
$org      = ( $org_raw && ! is_array($org_raw) ) ? esc_html($org_raw)
          : ( is_array($org_raw) ? f17s2_val($org_raw) : esc_html( get_post_field('post_title', $post_id) ) );
$ref_no   = f17s2_val( get_field( 's2_ref_no', $post_id ) );
$standard = f17s2_val( get_field( 's2_standard', $post_id ) );
$location = f17s2_val( get_field( 's2_location', $post_id ) );

$planned  = get_field( 's2_planned_monthyear', $post_id );
$executed = get_field( 's2_Executed_monthyear_copy', $post_id );
$matrix   = get_field( 's2_area_process', $post_id );

$plan_labels = ['Stage I' => 's2_stage_i', 'Stage 2' => 's2_stage_2', '1st Surveillance' => 's2_1st_Surveillance', '2nd Surveillance' => 's2_2nd_Surveillance', 'Re-Certification' => 's2_Re-Certification'];
?><!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<h2>F-17s2 &nbsp;|&nbsp; QMS Surveillance Year 2 &nbsp;|&nbsp; Version 1.00</h2>
...
<table>
    ...
    <?php foreach ( $plan_labels as $label => $key ) : ?>
        <tr>
            ...
            <td><?= is_array($planned) ? f17s2_val( $planned[$key] ?? '' ) : '-' ?></td>
            <td><?= is_array($executed) ? f17s2_val( $executed[$key] ?? '' ) : '-' ?></td>
        </tr>
    <?php endforeach; ?>
    </tbody>
</table>
