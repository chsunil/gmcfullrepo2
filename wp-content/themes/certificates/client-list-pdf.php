...
$assigned_employee = isset($_GET['assigned_employee']) ? sanitize_text_field($_GET['assigned_employee']) : '';
...
} elseif ($assigned_employee) {
    // If admin/manager and a filter is selected
    $args['meta_query'] = array(
        array(
            'key'   => 'assigned_employee',
...
// Get available auditors for the dropdown
$auditors = get_users(array('role' => 'auditor'));
?>
                                                ...
                                                <select name="assigned_employee" class="form-select me-2" onchange="this.form.submit()">
                                                    ...
                                                    <?php foreach ($auditors as $auditor) : ?>
                                                        <option value="<?php echo esc_attr($auditor->ID); ?>" <?php selected($assigned_employee, $auditor->ID); ?>>
                                                            <?php echo esc_html($auditor->display_name); ?>
                                                        </option>
                                                    <?php endforeach; ?>
                                                </select>
                                            <?php endif; ?>
                            ...
                            <th>Actions</th> 
                                        ...
                                        while ($clients->have_posts()) {
                                            ...
                                            if (in_array('administrator', $roles)) {
                                                ...
                                                $delete_btn = sprintf(
                                                    ' <button type="button" class="btn btn-danger btn-sm ms-1 delete-client-btn" data-post-id="%d" data-client-name="%s" data-nonce="%s" title="Delete Client"><span class="fas fa-trash"></span>Delete</button>',
                                                    $post_id,
                                    ...
                                    <?php echo '</td>';
                                        ...
                                        echo '<a class="page-link" href="' . esc_url(add_query_arg('paged', max(1, $paged - 1))) . '"><i class="icon-base bx bx-chevrons-left icon-sm"></i></a>';
                                        echo '</li>';
                                        ...
                                        echo '<a class="page-link" href="' . esc_url(add_query_arg('paged', min($clients->max_num_pages, $paged + 1))) . '"><i class="icon-base bx bx-chevrons-right icon-sm"></i></a>';
                                        echo '</li>';
